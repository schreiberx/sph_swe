 ...
 * Precompiler flags:
 *
 * - SWEET_THREADING: Activate (set to 1) to support multi-threaded memory allocation
 *
 * - NUMA_BLOCK_ALLOCATOR_TYPE: Define granularity of allocation
 *
 * 0: default allocator
 *
 * 1: NUMA domains
 ...
 *
 * 4: High-memory bandwidth allocation
 *    -> Same as (3), but with hbw allocation
 *    -> KNL specific allocation
 */


#if !(NUMA_BLOCK_ALLOCATOR_TYPE >= 0 && NUMA_BLOCK_ALLOCATOR_TYPE <= 4)
...
#if NUMA_BLOCK_ALLOCATOR_TYPE == 4
#	include <hbwmalloc.h>
#endif
...
private:
	...
	int& getThreadLocalDomainIdRef()
	{
		 ...
		 * (We use the thread_local attribute)
		 */
...
#elif  NUMA_BLOCK_ALLOCATOR_TYPE == 3 || NUMA_BLOCK_ALLOCATOR_TYPE == 4
...
//				std::cout << "cleaning up " << g.free_blocks.size() << " blocks of size " << g.block_size << std::endl;
				...
				for (auto& b : g.free_blocks)
				{
					...
					::free(b);
#elif NUMA_BLOCK_ALLOCATOR_TYPE == 4
					hbw_free(b);
#else
...
#	pragma omp critical
		...
		if (data != nullptr)
			return data;
#if NUMA_BLOCK_ALLOCATOR_TYPE == 3
		int retval = posix_memalign((void**)&data, 4096, i_size);
#else
		int retval = hbw_posix_memalign((void**)&data, 4096, i_size);
#endif
		if (retval != 0)
		{
...
public:
	...
	#if NUMA_BLOCK_ALLOCATOR_TYPE == 1 || NUMA_BLOCK_ALLOCATOR_TYPE == 3 || NUMA_BLOCK_ALLOCATOR_TYPE == 4
#if SWEET_THREADING || SWEET_REXI_THREAD_PARALLEL_SUM
	#pragma omp critical
#endif
	#endif
